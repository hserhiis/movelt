...
export default function Header() {
  ...
  const homeLink = useMemo(() => {
    ...
    return userProfile.role === 'driver' ? '/driver/dashboard' : '/client/dashboard';
  }, [userProfile]);
    ...
    <header className="bg-card/80 backdrop-blur-sm sticky top-0 z-50 border-b border-border">
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href={homeLink} className="flex items-center gap-2 text-2xl font-bold text-primary font-headline">
              <Truck />
              <span>MoveIt</span>
          </Link>
